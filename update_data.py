...
print("📊 FinanceDataReader를 통해 시장 종목 데이터를 불러옵니다...")
...
# 2. KRX 전체 종목 가져오기 (시가총액 상위 300개 우량주 집중 스캔)
df_krx = fdr.StockListing('KRX')
df_krx = df_krx[df_krx['Code'].str.match(r'^\d{6}$')]
df_top = df_krx.head(300).copy()

data_list = []
print(f"🚀 총 {len(df_top)}개 종목의 과거 주가 및 지표 계산 시작...")
...
for idx, row in df_top.iterrows():
    ...
    try:
        ...
        # 가상 PER, PBR, 시총 추출 (또는 기본값 부여)
        market_cap = float(row.get('Marcap', 500000000000) / 100000000) if 'Marcap' in row and pd.notna(row['Marcap']) else 3000
...
if len(data_list) > 0:
    ...
    print(f"✅ 수집 완료! 총 {len(df_master)}개 종목의 데이터가 갱신되었습니다.")
else:
    print("❌ 수집된 데이터가 없습니다.")
